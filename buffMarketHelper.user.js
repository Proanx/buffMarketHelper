...
(function () {
    ...
    function updateRate(force) {
        ...
        if ((!steamConnection) && steamConnection != undefined) {
            showMessage("无网络时无法修改汇率", "货币设置未保存", 'warning');
            return;
        }
        GM_xmlhttpRequest({
            ...
            onerror: function (err) {
                ...
                showMessage("更新汇率失败", errorTranslator(err).statusText + "<br/>货币设置未保存", 'error');
            },
            ontimeout: function () {
                failedSteamConnection();
                let err = { "status": 408, "statusText": "连接steam超时，请检查steam市场连接性" };
                console.log("更新汇率超时：", err);
                showMessage("更新汇率超时", err.statusText + "<br/>货币设置未保存", 'error');
            }
        });
    }
